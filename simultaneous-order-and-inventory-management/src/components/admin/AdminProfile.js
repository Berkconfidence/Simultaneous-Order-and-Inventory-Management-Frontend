...
import "../profile/ProgressCircle.css";
import "../profile/Order.css";
...
function AdminProfile() {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const response = await fetch(`/customers/${adminId}`);
                if (response.ok) {
        ...
        const fetchOrderProducts = async () => {
            try {
                const response = await fetch(`/orders/customer/${adminId}`);
                if (response.ok) {
                    const data = await response.json();
        ...
        fetchProducts();
        fetchOrderProducts();
    }, [adminId], [products], [orderProducts]);
